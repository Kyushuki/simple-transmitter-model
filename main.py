...
class Model:
    ...
    def start_signal_phase(self) -> complex:
        """
        Задаёт случайную начальную фазу для сигнала
        """
        phi = np.random.uniform(0, 2 * np.pi)
        phase = np.exp(1j * phi)
        return complex(phase)

    def emitter_qpsk(self) -> list[complex]:
        """
        Моделирует работу передатчика с QPSk модуляцией
        """
        ...
        # кодирование, упаковка, модуляция
        ...
        # вывод рисунка созвездия модуляции на разных этапах "передачи"
        ...
        # поворот созвездия случайной начальной фазой
    ...
    def receiver_qpsk(self, complex_mess: list[complex]):
        """
        Моделирует работу приёмника с QPSk модуляцией
        """
        # демодуляция и распаковка
        counter = 0
        packed_mess = self.demodulator.demodulate_qpsk(complex_mess)
        byte_mess, coder = self.depacking.depacket(packed_mess)
        # выбор кодировки для декодирования и цикл в случае провала демодуляции (пришла ошибка вместо необходимой кодировки)
        if coder == "error":
            while coder == "error":
                packed_mess = self.demodulator.demodulate_qpsk(complex_mess)
                byte_mess, coder = self.depacking.depacket(packed_mess)
...
if __name__ == "__main__":
    ...
    a = model.emitter_qpsk()
    print(a)
    b = model.receiver_qpsk(a)
